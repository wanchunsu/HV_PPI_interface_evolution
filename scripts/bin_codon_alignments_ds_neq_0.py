...
def annotate_residues_with_category(exo_residues, endo_residues, mim_residues, surface_residues, buried_residues, all_exo_residues, all_endo_residues, dna_msas):
	...
	# 			print(len(dict_of_targprot_residue_annos[t][r]))

	#print(dict_of_targprot_residue_annos)
	return dict_of_targprot_residue_annos

def get_codon_mappings(aa_msas, dna_msas, site_specific_dn_ds_folder):
	...
	for targ_prot_aa_msa_fi in os.listdir(aa_msas) :
		if targ_prot_aa_msa_fi.endswith('_msa.fasta'):
			...
			dict_of_ds_pos = annotate_codons_that_have_well_defined_dnds(site_specific_dn_ds_folder, aa_msas, targ_prot_id)
			...
			while aa_pos <= len(aa_alignment[0]):
				...
				for org_ind in range(len(headers)):
					...
					codon_mappings_dict[targ_prot_id][aa_pos][headers[org_ind]] = (list_of_aa_residues_at_pos[org_ind], str(list_of_codons_at_pos[org_ind].seq), dict_of_ds_pos[aa_pos]) #store 
...
def annotate_codons_that_have_well_defined_dnds(site_specific_dn_ds_folder, aa_msas, targ_prot_id):
	""" Get only aa msa positions & corresponding codons with non-negative dS values (we will ignore those with either no synonymous substitutions or only one non-gap residues)
	

	"""
	# make dict to store the site-specific ds values at each position in the msa  (this will be checking for special cases where we could have non-well-defined dn/ds cases)
	site_specific_dn_ds_fi = osp.join(site_specific_dn_ds_folder, targ_prot_id + '.parsed_dnds.csv')
	dict_of_ds_pos = {}


	with open(site_specific_dn_ds_fi) as dnds_csv:
		...
		for row in csv_reader: 
			...
			ds = float(row[1])
			#dn = float(row[2])
			dict_of_ds_pos[site_pos] = ds

	
	
	return dict_of_ds_pos
...
def map_translated_aa_residues_to_msa_aligned_residues(translated_aa_tblastn_alignment_with_orig_prot, codon_mappings_dict, targ_prot, aa_msa_file):
	...
	for msa_alignment_pos in codon_mappings_dict[targ_prot]:
		if ind_translated_aa < len(translated_aa_tblastn_alignment_with_orig_prot): #if we're past the last residue in the tblastn alignment, then that means everything else is a gap in the msa
			(aa_msa_residue, dna_msa_codon, ds) = codon_mappings_dict[targ_prot][msa_alignment_pos][targ_prot] #get the aa and corresponding codon at the given msa position
...
def map_dnds_to_targ_prot_residue_positions(dict_of_alignments, aa_msas, codon_mappings_dict):
	...
	for targ_prot in dict_of_alignments:
		...
		for targ_prot_res, translated_aa_res, ind_translated_aa in zip(targ_prot_aligned, translated_aa_aligned, dict_of_translated_cds_pos_and_aa_codon):
			...
			(aa_msa_residue, dna_msa_codon, ds) = dict_of_translated_cds_pos_and_aa_codon[ind_translated_aa][targ_prot] #for verification purposed
			if targ_prot_res == '-': #gap in targ prot alignment, so skip b/c no residue to store
				...
				if aa_msa_residue != translated_aa_res: #just to verify that things worked
					print('incorrect')
				if ds != 0: #only store if ds is non-negative
...
def bin_residues(dict_of_targ_prot_aa_codons, dict_of_targprot_residue_annos):
	...
	for resid_a in dict_of_all_residues_codons_in_each_category:
		print(f"{resid_a}: {len(dict_of_all_residues_codons_in_each_category[resid_a]['human'])}")
...
def output_binned_codons(dict_of_all_residues_codons_in_each_category, output_folder):
	...
	for residue_anno in dict_of_all_residues_codons_in_each_category:
		with open(osp.join(output_folder, residue_anno + '.binned_codons_ds_neq_0.fasta'), 'w') as of:
...
def main():
	...
	site_specific_dn_ds_folder = osp.join(script_dir, '..', 'data', 'dnds', 'hyphy_out_species_tree')
	...
	codon_mappings_dict = get_codon_mappings(aa_msas, dna_msas, site_specific_dn_ds_folder)
	...
	# Binning residues into the 7 residue categories (exo, endo, mimicry, surface, buried, all_exo, all_endo)
