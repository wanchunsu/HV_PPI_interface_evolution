...
def get_bootstrap_standard_err(dict_of_divergence_values, residue_type, gaps_or_subs, targ_prot, repeats = 1000):
	...
	residue_type_list = residue_type + '_lists'
	...
	for i in range(repeats):
		...
		for org in dict_of_divergence_values:
			binary_list = dict_of_divergence_values[org][residue_type_list][targ_prot][list_type]
...
def main():
	...
	divergence_values = osp.join(script_dir, '..', 'data', 'homologs_and_conservation', ortholog_folder_label, folder_name, 'per_protein_divergence_with_close_species.json')
	...
	output_standard_errs_json = osp.join(script_dir, '..', 'data', 'homologs_and_conservation', ortholog_folder_label, folder_name, 'per_protein_standard_errors.json')
	...
	for residue_type in list_of_residue_categories:
		residue_type_list = residue_type + '_lists'
		dict_of_bootstrap_standard_errs['subs'][residue_type] = {}
		dict_of_bootstrap_standard_errs['gaps'][residue_type] = {}
		dict_of_bootstrap_standard_errs['all_div'][residue_type] = {}

		first_org = list(dict_of_divergence_values.keys())[0]
		for targ_prot in dict_of_divergence_values[first_org][residue_type_list]: #the targ_prots are the same in every organism, so just picked the first organism in the dict
			res_type_subs_standard_err = get_bootstrap_standard_err(dict_of_divergence_values, residue_type, 'subs', targ_prot, repeats = 1000)
			res_type_gaps_standard_err =  get_bootstrap_standard_err(dict_of_divergence_values, residue_type, 'gaps', targ_prot, repeats = 1000)
			res_type_div_standard_err =  get_bootstrap_standard_err(dict_of_divergence_values, residue_type, 'all_div', targ_prot, repeats = 1000)

			dict_of_bootstrap_standard_errs['subs'][residue_type][targ_prot] = res_type_subs_standard_err
			dict_of_bootstrap_standard_errs['gaps'][residue_type][targ_prot] = res_type_gaps_standard_err
			dict_of_bootstrap_standard_errs['all_div'][residue_type][targ_prot] = res_type_div_standard_err
