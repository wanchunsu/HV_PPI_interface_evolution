...
Compute overall evolutionary scores from default Rate4Site output
=================================================================
...
1) [THIS SCRIPT] Z-score normalization (mean = 1, std = 0) Default rate4site output. Will be using parse_rate4site_scores_into_dict with file_ext = '.scores'. This will parse z-score-normalized scores into dict.
...
def parse_rate4site_scores_into_dict(rate4site_results_folder, file_ext):
	...
	Args:
		normalized_rate4site_results_folder: folder storing rate4site scores 
		file_ext: file extension specifiying the type of scores we want to use (.scores)
	Return:
		dict_of_rate4site_scores: dictionary storing rate4site normalized scores (z-score normalized), normalization is done automatically by rate4site
	
	"""
	dict_of_rate4site_scores ={}
	for targ_prot_rate4site_results in os.listdir(rate4site_results_folder):
		if targ_prot_rate4site_results.endswith(file_ext):
			...
			with open(osp.join(rate4site_results_folder, targ_prot_rate4site_results)) as res:
				for line in res:
					#print(line)
					if not line.startswith("#") and not line == "\n" : #these are the headers and empty lines respectively
						...
						dict_of_rate4site_scores[targ_prot][res_pos] = res_score
...
def main():
	...
	rate4site_overall_scores_for_residue_categories = osp.join(output_folder, folder_name + "_" + rate4site_results + ".json")
	...
	rate4site_all_scores_for_residue_categories = osp.join(rate4site_all_residue_category_scores_folder, folder_name + "_" + rate4site_results + "_all_residue_scores.json")
	...
	rate4site_per_protein_scores = osp.join(rate4site_per_protein_folder, folder_name + '_' + rate4site_results + '_per_protein_scores.json')


	############ Using Z-score normalized scores --done automatically by rate4site ############
	print('############### Using Z-score normalized rate4site scores ###############') 
	print(f"Getting scores for {folder_name}, {rate4site_results} . . . \n")

	#format scores for all residues on the target proteins into a dictionary
	dict_of_rate4site_scores = parse_rate4site_scores_into_dict(rate4site_results_folder, '.scores') #we want to use the rate4site normalized scores (note these are z-score normalized)
	...
	with open(rate4site_overall_scores_for_residue_categories, 'w') as output_fi:
		json.dump(scores, output_fi)

	#output all the residue scores dict
	with open(rate4site_all_scores_for_residue_categories, 'w') as all_residue_scores_output_fi:
		json.dump(all_residue_scores, all_residue_scores_output_fi)

	#output per protein scores for each residue category
	with open(rate4site_per_protein_scores, 'w') as per_protein_scores_output_fi:
